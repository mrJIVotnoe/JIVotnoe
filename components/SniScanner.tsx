...
import { Activity, Check, X, RefreshCw, Radar, Play } from 'lucide-react';
import { SNI_DOMAINS } from '../data';
...
export const SniScanner = ({ onSelect }: { onSelect?: (domain: string) => void }) => {
  ...
  const checkDomain = async (domain: string) => {
    ...
    try {
        // no-cors allows opaque response from any origin without throwing CORS error
        // if connection succeeds (even 404), promise resolves
        // if network error/blocked, promise rejects
